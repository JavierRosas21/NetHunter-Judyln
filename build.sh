...
# ---------------------------------------------------------------
# Local overrides are supported via env vars or local.config.
# Example: KDIR=/path/to/android_kernel_lge_sdm845
# ---------------------------------------------------------------

# Build directory
BUILD_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
DEFAULT_KERNEL_DIR="${BUILD_DIR}/../android_kernel_lge_sdm845"
...
function edit_config() {
    ...
    # JAVI MOD: Siempre decir que NO queremos editar manualmente para automatizar
...
function apply_patch() {
    ...
    if [ $? == 0 ]; then
        printf "\n"
        # JAVI MOD: Aplicar parche automáticamente si el test pasa
...
function make_kernel() {
    ...
    if [ ! "$cfg_done" = true ]; then
        # JAVI MOD: Saltar pregunta y solo construir
        # if ask "Edit the kernel config?" "Y"; then
        #    info "Creating custom  config" 
        #    make -C $KDIR O="$KERNEL_OUT" $cc $CONFIG $CONFIG_TOOL 
        # fi
    ...
    # JAVI MOD: No preguntar si guardar config
...
do
    ...
    # Prefer explicit overrides; otherwise use sibling kernel dir if available.
    CONFIG_DEFAULT_KDIR="$( cd "${BUILD_DIR}/.." >/dev/null 2>&1 && pwd )"
    if [ -n "${KERNEL_DIR}" ]; then
        KDIR="${KERNEL_DIR}"
    elif [ "${KDIR}" = "${CONFIG_DEFAULT_KDIR}" ] && [ -d "${DEFAULT_KERNEL_DIR}" ]; then
        KDIR="${DEFAULT_KERNEL_DIR}"
    fi
    KERNEL_DIR="${KDIR}"
    ...
    if [ -z "${CONFIG}" ] || [ "${CONFIG}" = "nethunter_defconfig" ]; then
        CONFIG="${DEFCONFIG}"
    fi
